...
listarProfesionales: function (arrayProfesionales) {
    for (let i = 0; i < arrayProfesionales.length; i++) {
      console.log(`id: ${arrayProfesionales[i].id}, especialidad: ${arrayProfesionales[i].especialidad} nombre: ${arrayProfesionales[i].nombre} valor consulta: ${arrayProfesionales[i].honorarioConsulta} puntaje: ${arrayProfesionales[i].puntaje}`);
    } // cuando presiono ctrl o alt Z para acomodar la linea me abre el panel de nvidia :(
  },
...
filtrarMejoresPuntajes: function () {
    ...
    for (let i = 0; i < this.profesionales.length; i++) {
      if (this.profesionales[i].puntaje >= 7) {
        puntajes.push(this.profesionales[i])
      }
    }
    return puntajes
  },
...
buscarPorId: function (idBuscado) {
    for (let i = 0; i < this.profesionales.length; i++) {
      if (this.profesionales[i].id == idBuscado) {
...
habilitarProfesional: function (id) {
    let habilitado = this.buscarPorId(id)
    for (let i = 0; i < this.profesionales.length; i++) {
        if (this.profesionales[i].estaHabilitado == false) {
            this.profesionales[i].estaHabilitado = true;
           }
       } return habilitado; 
   },
...
corregirNombres : function () {
    for(let i=0; i<this.profesionales.length;i++){
      this.profesionales[i].nombre=this.profesionales[i].nombre[0].toUpperCase()+this.profesionales[i].nombre.slice(1)
      }
...
console.log(appProfesionales.buscarPorId(2));
console.log(o);

console.log(v, oo + "  E. habilitarProfesional");
console.log(appProfesionales.habilitarProfesional(3));
console.log(o);
